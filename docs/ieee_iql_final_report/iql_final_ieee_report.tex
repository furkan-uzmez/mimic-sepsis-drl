...
Önerilen iş akışı, yatak başı gerçek zamanlı bir tedavi modülü sunmaktan ziyade, retrospektif politika değerlendirmesi ve model karşılaştırmaları için metodolojik olarak izole edilmiş bir analiz ortamı teşkil eder. Model sınıfı modelsiz (model-free), politika dışı (off-policy) ve çevrimdışı pekiştirmeli öğrenmedir. Muhafazakar Q-öğrenme (conservative Q-learning, CQL), davranış kısıtlı Q-öğrenme (batch-constrained Q-learning, BCQ) ve örtük Q-öğrenme aileleri tasarım aşamasında değerlendirilmiştir \cite{cql,bcq,iql}. Final rapor IQL'ye odaklanır; çünkü IQL veri dışı aksiyon maksimumu almadan politika çıkarır. Kodun MPS ve CUDA üzerinde aynı eğitim mantığıyla çalışması istenir; donanım farkı deney kararına dönüşmemelidir.
...
Sabit parametreler korunmuştur; böylece final tarama yalnız ödül (reward), öğrenme oranı rejimi, ekspektil ve sıcaklık etkisini ölçer. Sabit parametreler, 18 konfigürasyonun etkisini ödül, öğrenme oranı ve IQL ayarları üzerinde izole etmeyi sağlar.
...
\section{Tohum Stratejisi (Seed Strategy) ve Karar Kuralı}
...
\begin{table}[t]
\centering
\caption{Referans politika teşhisleri. FQE/WIS karşılaştırması Tablo~\ref{tab:baseline}'de yalnız seçili IQL için verilir; burada referans politikaların destek ve klinisyen uyumu bağlamı gösterilir.}
\label{tab:reference_policies}
\begin{tabular}{lrr}
\toprule
Politika & ESS & Uyum \\
\midrule
Klinisyen tekrar oynatımı & 100.0 & 1.000 \\
Tedavisiz & 100.0 & 0.120 \\
Davranış klonlama & 92.0 & 0.580 \\
Seçili IQL & 29.4 & 0.414 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
 ...
 \caption{Final seçili IQL politikası ve referans politika gösterimi. FQE ve WIS yalnız seçili IQL için gerçek Aşama 2 OPE çıktısıdır; klinisyen tekrar oynatımı, tedavisiz politika ve davranış klonlama çubukları bağlamsal referans olarak verilmiştir.}
