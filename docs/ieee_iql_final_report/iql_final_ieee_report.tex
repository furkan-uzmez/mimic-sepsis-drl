...
Problem sonlu ufuklu bir Markov karar süreci olarak modellenir \cite{sutton2018rl}:
\begin{equation}
M=(S,A,P,R,\gamma).
\end{equation}
...
\end{equation}
$\tau$ 1'e yaklaştıkça değer fonksiyonu davranış verisi içinde daha iyi görünen aksiyonların üst expectile değerine (upper expectile) yaklaşır. Q fonksiyonu ise veri dışı aksiyon maksimumu yerine öğrenilen durum-değer hedefiyle (state-value target) güncellenir:
\begin{equation}
...
Sabit parametreler korunmuştur; böylece final sweep yalnız ödül (reward), öğrenme oranı rejimi, expectile ve temperature etkisini ölçer. Sabit parametreler, 18 konfigürasyonun etkisini ödül, öğrenme oranı ve IQL ayarları üzerinde izole etmeyi sağlar.

\begin{algorithm}[t]
\caption{Final IQL Sweep ve Raporlama İş Akışı}
\label{alg:workflow}
\begin{algorithmic}[1]
\STATE Replay dataset, bölme ve ön işleme (replay, split, preprocessing) ve zamansal sözleşmeleri doğrula
...
\STATE Baseline karşılaştırmaları (baseline comparisons), grafikler ve yeniden üretilebilirlik çıktıları üret
\end{algorithmic}
\end{algorithm}

\section{Seed Stratejisi (Seed Strategy) ve Karar Kuralı}
Tek seed'li (single-seed) değerlendirme, offline RL modellerinde kararlı politika seçimi için yetersiz kanıt üretir. Başlatma (initialization), mini-yığın (minibatch) sırası ve stokastik optimizasyon (stochastic optimization) aynı konfigürasyonun FQE/WIS değerini değiştirebilir. Aşama 1'de 18 konfigürasyon seed 42 ile taranır; ardından altı finalist seed 123 ve 456 ile yeniden değerlendirilir. Böylece tüm $2\times3\times3\times3=54$ çalıştırma (run) yerine 30 çalıştırma çalıştırılır. Bu tasarım hesaplama maliyetini düşürür, fakat final adayları üç seed üzerinden raporladığı için seed varyansı (seed variance) görünür kalır.
...
Bir konfigürasyon final aday olmadan önce doğrulama performansı rekabetçi olmalı, ESS çok düşük olmamalı, FQE sonucu WIS ile tamamen çelişmemeli, aksiyon dağılımı klinik olarak makul olmalı, low-support action oranı kabul edilebilir düzeyde olmalı, farklı seed'lerde (seeds) performans tamamen kararsız olmamalı ve test set seçim sürecinde kullanılmamış olmalıdır.
...
Finalist havuzu (finalist pool) tek bir skora göre seçilmez. Klinik offline learning'de yüksek değer tahmini (value estimate), düşük behavior support ile birleştiğinde güvenilir değildir \cite{gottesman2019guidelines}. Yalnız en yüksek FQE veya WIS kullanılsaydı, düşük ESS ya da zayıf clinician exact-bin agreement taşıyan bir politika Aşama 2'ye geçebilirdi. Çeşitlilik slotları da bu yüzden korunur: Aşama 2 yalnız SOFA-konservatif adayları tekrar etmez; seyrek ödülün ve baseline settings'in sınırlarını da gösterir.
...
\begin{table}[t]
\centering
\caption{Kazanan konfigürasyonun test diagnostics (test teşhisleri). WIS confidence interval, 50 hasta-düzeyi patient-level bootstrap resampling ile hesaplanmıştır.}
\label{tab:baseline}
\begin{tabular}{lr}
\toprule
Metrik & Sonuç \\
...
\begin{figure}[t]
 ...
 \caption{Final seçili IQL politikası ve baselines için WIS ve ESS teşhisleri. Behavior Cloning (BC) WIS değeri yüksek görünse de ESS=1.6 olduğu için güvenilir politika üstünlüğü kanıtı olarak yorumlanmaz.}
...
\begin{figure}[t]
 ...
 \caption{Off-Policy Evaluation (OPE) metriklerine göre finalist sıralaması. Grafik, Tablo~\ref{tab:stage2}'deki bileşik skorun FQE, WIS, ESS ve destek göstergeleriyle birlikte okunması gerektiğini gösterir.}
 \label{fig:ope_metric_ranking}
\end{figure}

\section{Aksiyon ve Güvenlik Teşhisleri (Action and Safety Diagnostics)}
Değer metriği (value metric) tek başına klinik politika güvenliği göstermez. Bir politika iyi FQE üretebilir, fakat gözlemsel veride nadir görülen sıvı-vazopresör kombinasyonlarına kayabilir. Bu yüzden action heatmap, behavior support mass, low-support rate, clinician agreement ve action entropy aynı anda okunur.
...
\begin{figure}[t]
 ...
 \caption{Final seçili IQL politikasının üç seed için patient-level bootstrap WIS confidence intervals (CI). Güven aralıkları 50 patient-level bootstrap resampling ile hesaplanmıştır ve düşük ESS nedeniyle temkinli yorumlanmalıdır.}
